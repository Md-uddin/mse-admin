...
export default function PolarChart({ chartData }) {
  ...
  React.useEffect(() => {
    if (showData) {
      ...
      let ctx = document
        .getElementById(`polarChart${showData.id}`)
        .getContext("2d");
  ...
  return (
    <>
      <div className="relative flex flex-col min-w-0 break-words bg-white w-full mb-6 shadow-lg rounded-xl pb-4">
        ...
        <div className="p-3 flex-auto justify-items-center align-items-center m-tb-3">
          {/* Chart */}
          <div className="relative h-350-px sm:h-auto">
            <canvas id={`polarChart${showData?.id}`}></canvas>
          </div>
        </div>
      </div>
    </>
